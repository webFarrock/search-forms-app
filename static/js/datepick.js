...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		...
		_generateMonth: function(elem, inst, year, month, renderer, first) {
			...
			// Generate weeks
			var weeks = '';
            //var curMonth = drawDate.getMonth()+2;

            //var month2Sighn =  curMonth < 10 ? '0'+curMonth : curMonth;
			for (var week = 0; week < numWeeks; week++) {
				...
				for (var day = 0; day < 7; day++) {
                    ...
                    if(inst.options.allowedDatesInited) {
                        //console.log(drawDate); 
                        //console.log(drawDate.getFullYear() + '-' + month2Sighn + '-' + curDate);
                        var moth = drawDate.getMonth() +1;
                        moth = moth < 10 ? '0'+moth : moth;

                        //console.log(drawDate.getFullYear() + '-' + moth + '-' + curDate );
                        selectable = selectable && -1 !== inst.options.allowedDates.indexOf(drawDate.getFullYear() + '-' + moth + '-' + curDate);
                    }
		...
		_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader) {
			...
			if (yearRange === 'any') {
				...
				if (start.getTime() < end.getTime()) {
					...
					var earlierLater = Math.floor((end - start));
